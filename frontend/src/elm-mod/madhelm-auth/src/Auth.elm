...
credHeader (Cred cred) =
    case cred of
        Just c ->
            [ Http.header "authorization" c.token ]

        -- Should be something like: ("Token " ++ cred.token)
        Nothing ->
            []
...
anonymousToken maybeToken =
    -- XXX should use the body to get accountID
    case maybeToken of
        Just token ->
            Cred (Just (Credentials "" token))
...
storeCred (Cred cred) =
    case cred of
        Just c ->
            State.store storageField (encodeCred c)
...
credExtractor email res =
    -- XXX should use the body to get accountID
    case Dict.get "set-authorization" res.headers of
        Just token ->
            Ok (Cred (Just (Credentials email token)))

        Nothing ->
            Err "no set-authorization header"
...
testSuite =
    T.describe "API tests"
        [ T.test "loadCred loads a credential" <|
            \_ ->
